...
public class SolarSystem extends Celestial {
	...
	public SolarSystem(float[] position, float[] rotation, float[] zDepths, int diameter, String[] texturePaths, int[] textureSizes, String name, int starSize) {
		super(null, position, rotation, zDepths, diameter, texturePaths, textureSizes, name);
	...
	public void updateView() {
		if (sceneNodes.isEmpty()) {
			for (int i = 0; i < texturePaths.length; i++) {
				...
				VBOSceneNode solarSystemSceneNode = new VBOSceneNode(shaderProgram, objData, getPosition(), 0.0f, zDepths[i], GL3bc.GL_TRIANGLES);
			...
			sceneNodes.put(name + "_Selection" + "_SolarSystemSceneNode", new VBOSceneNode(shaderProgram, objData, getPosition(), 0.0f, ZDepth.SELECTION.getDepth(), GL3bc.GL_TRIANGLES));
