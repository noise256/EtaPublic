...
public class TradeCompany extends Faction implements Agent {
	...
	private TradeShip createTradeShip() {
		if (balance >= ShipProfile.DEFAULT.price()) {
			...
			TradeShip tradeShip = new TradeShip(new float[] {MathBox.nextFloat() * systemSpread - systemSpread / 2, MathBox.nextFloat() * systemSpread - systemSpread / 2}, new float[] {1.0f, 0.0f}, new float[] {ZDepth.SHIP.getDepth()}, 20, new String[] {"trade_ship_dark_blue.png"}, new int[] {10}, new float[] {0.0f, 0.0f}, PhysicalProfile.DEFAULT, ShipProfile.DEFAULT, this); 
