...
public abstract class PhysicalObject extends GameObject implements Controllable {
	...
	public PhysicalObject(GameObject parent, float[] position, boolean positionRelative, float[] rotation, float[] zDepths, int diameter, String[] texturePaths, int[] textureSizes, float[] velocity, PhysicalProfile physicalProfile) {
		super(parent, position, positionRelative, rotation, zDepths, diameter, texturePaths, textureSizes);
	...
	private void update() {
		adjustRelativePosition(velocity);
	}
	...
	private void turnTo(float[] orientation) {	
		float turningAngle = MathBox.vectorToAngle(orientation) - getRotationAngle();
