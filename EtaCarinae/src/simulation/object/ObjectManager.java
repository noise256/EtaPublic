...
public class ObjectManager {
	...
	public RenderEvent getObjectDisplayUpdateEvent() {
		...
		//return SceneNodes sorted back to front
		return new RenderEvent(this, sortSceneNodesByDepth(objectNodes));
	}
	
	public RenderEvent getUIDisplayUpdateEvent() {
		ArrayList<SceneNode> uiNodes = new ArrayList<SceneNode>();
		...
		for (UIPanel uiPanel : UIPanelManager.getUIPanels()) {
			...
			if (uiPanel.getVisible()) {
				for (SceneNode sceneNode : uiPanel.getView()) {
					if (sceneNode.isVisible()) {
						uiNodes.add(sceneNode);
					}
				}	
			}
		}
		
		return new RenderEvent(this, uiNodes);
	}
	
	private ArrayList<SceneNode> sortSceneNodesByDepth(ArrayList<SceneNode> sceneNodes) {
		if (sceneNodes.size() <= 1) {
			return sceneNodes;
		}
		
		ArrayList<SceneNode> sorted = new ArrayList<SceneNode>();
		
		SceneNode pivot = sceneNodes.remove(sceneNodes.size()/2);
		
		ArrayList<SceneNode> lesser = new ArrayList<SceneNode>();
		ArrayList<SceneNode> greater = new ArrayList<SceneNode>();
		
		for (SceneNode sceneNode : sceneNodes) {
			if (((VBOSceneNode) sceneNode).getNodeDepth() <= ((VBOSceneNode) pivot).getNodeDepth()) {
				lesser.add(sceneNode);
			}
			else {
				greater.add(sceneNode);
			}
		}
		
		sorted.addAll(sortSceneNodesByDepth(lesser));
		sorted.add(pivot);
		sorted.addAll(sortSceneNodesByDepth(greater));
	...
	private void handleMouseRelease(Input release) {
		...
		if (uiCollider != null) {
			...
			if (release.getButton() == MouseEvent.BUTTON1) {
				if (objectCollider == null || (objectCollider != null && objectCollider.getUIStrings() != null)) {
