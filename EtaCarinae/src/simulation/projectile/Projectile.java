...
public class Projectile extends PhysicalObject implements PhysicalAgent {
	...
	public Projectile(float[] position, float[] rotation, float[] velocity, PhysicalProfile physicalProfile, ProjectileProfile projectileProfile, int life) {
		super(null, position, false, rotation, projectileProfile.getZDepths(), projectileProfile.getDiameter(), projectileProfile.getTexturePaths(), projectileProfile.getTextureSizes(), velocity, physicalProfile);
	...
	public void updateView() {
		if (sceneNodes.isEmpty()) {
			...
			for (int i = 0; i < texturePaths.length; i++) {
				...
				VBOSceneNode projectileSceneNode = new VBOSceneNode(shaderProgram, objData, getPosition(), getRotationAngle(), zDepths[i], GL3bc.GL_TRIANGLES);
