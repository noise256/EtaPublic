...
public class Simulation extends SimulationManager {
	...
	//TODO set planet rotations to face stars
	@Override
	protected void generate() {
		...
		float[] systemPosition = new float[] {2000.0f, 0.0f};
		
		String starTexture = CelestialTexture.values()[MathBox.nextInt(CelestialTexture.values().length)].path();
		SolarSystem originSystem = new SolarSystem(systemPosition, new float[] {1.0f, 0.0f}, new float[] {ZDepth.SYSTEM_WELL.getDepth(), ZDepth.STAR.getDepth()}, systemRadius*2, new String[] {"system_well_low_opacity.png", starTexture}, new int[] {systemRadius*2, starRadius*2}, "System " + Integer.toString(0), starRadius*2);
		...
		for (int i = 1; i < numSystems; i++) {
			...
			SolarSystem randomSystem = new SolarSystem(systemPosition, new float[] {1.0f, 0.0f}, new float[] {ZDepth.SYSTEM_WELL.getDepth(), ZDepth.STAR.getDepth()}, systemRadius*2, new String[] {"system_well_low_opacity.png", starTexture}, new int[] {systemRadius*2, starRadius*2}, "System " + Integer.toString(i), starRadius*2);
		...
		for (SolarSystem solarSystem : solarSystems) {
			...
			for (int j = 0; j < numPlanets; j++) {
				...
				Planet planet = new Planet(solarSystem, planetPosition, new float[] {1.0f, 0.0f}, new float[] {ZDepth.PLANET.getDepth()}, planetSize*2, new String[] {"ice_planet.png"}, new int[] {planetSize*2}, "Planet " + (j + 1), solarSystem, fertilities);
		...
		TradeShip controlledShip1 = new TradeShip(new float[] {0.0f, 0.0f}, new float[] {1.0f, 1.0f}, new float[] {ZDepth.SHIP.getDepth()}, 100, new String[] {"trade_ship_dark_blue.png"}, new int[] {100}, new float[] {0.0f, 0.0f}, PhysicalProfile.DEFAULT, ShipProfile.DEFAULT, null);
		Weapon controlledShip1Weapon1 = new Weapon(controlledShip1, new float[] {2.0f, 10.0f}, new float[] {1.0f, 0.0f}, WeaponProfile.DEFAULT, ProjectileProfile.DEFAULT_DUMB);
		Weapon controlledShip1Weapon2 = new Weapon(controlledShip1, new float[] {2.0f, -10.0f}, new float[] {1.0f, 0.0f}, WeaponProfile.DEFAULT, ProjectileProfile.DEFAULT_DUMB);
		controlledShip1.addWeapon(controlledShip1Weapon1);
		controlledShip1.addWeapon(controlledShip1Weapon2);
		
		addAgent(controlledShip1);
		addGameObject(controlledShip1);
		addGameObject(controlledShip1Weapon1);
		addGameObject(controlledShip1Weapon2);
		
		TradeShip controlledShip2 = new TradeShip(new float[] {100, 0}, new float[] {1.0f, 0.0f}, new float[] {ZDepth.SHIP.getDepth()}, 100, new String[] {"trade_ship_dark_blue.png"}, new int[] {100}, new float[] {0.0f, 0.0f}, PhysicalProfile.DEFAULT, ShipProfile.DEFAULT, null);
		addAgent(controlledShip2);
		addGameObject(controlledShip2);
		
		System.err.println("Finished generating controlled ships.");
		
		TimerManager.addTimer("mainTimer", new Timer(1000));
	}
}
