...
public class Renderer implements GLEventListener, Observer<RenderEvent> {
	...
	private ArrayDeque<ArrayList<SceneNode>> renderEvents = new ArrayDeque<ArrayList<SceneNode>>();
...
//		drawDebugCube(0f, 0f, 0f);
		...
		while (!renderEvents.isEmpty()) {
			ArrayList<SceneNode> currentSceneNodes = renderEvents.pop();
	...
	public void update(RenderEvent event) {
		renderEvents.add(event.getSceneNodes());
	}
